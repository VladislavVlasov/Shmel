(() => {
    ...
    function scrollActive() {
        ...
        sections.forEach((current => {
            ...
            if (scrollY > sectionTop && scrollY <= sectionTop + sectionHeight) document.querySelector(".menu__body a[href*=" + sectionId + "]").classList.add("_navigator-active"); else document.querySelector(".menu__body a[href*=" + sectionId + "]").classList.remove("_navigator-active");
        }));
    }
    window.addEventListener("scroll", scrollActive);
    document.addEventListener("DOMContentLoaded", scrollActive);
    const videoPlayButtons = document.querySelectorAll(".slide-play");
    videoPlayButtons.forEach((function(button) {
        button.addEventListener("click", (function(e) {
            const parent = button.parentElement;
            const linkToVideo = button.getAttribute("href");
            const id = YouTubeGetID(linkToVideo);
    ...
    function YouTubeGetID(url) {
        ...
        if (void 0 !== url[2]) ID = url[2]; else ID = url;
        return ID;
    }
    ...
    function generateURL(id) {
        var query = "rel=0&showinfo=0&autoplay=1", sep = id.indexOf("?") > -1 ? "&" : "?";
        return "https://www.youtube.com/embed/" + id + sep + query;
    }
    window["FLS"] = true;
    isWebp();
    menuInit();
